...
if ( isset( $_GET['post_parent'] ) ) {
	...
	try {
		...
		if ( false === $filtered_get_post_parent || null === $filtered_get_post_parent ) {
...
if ( $redirect ) {
	...
	if ( null !== $the_post && $the_post->post_parent > 0 ) {
		/* Redirect to parent group if id of child page was given */
		$redirect_url = Main::get_groups_list_admin_url( $the_post->post_parent );
	} else {
		$redirect_url = Main::get_groups_list_admin_url();
	}
...
require_once PLUGIN_PATH . 'src/admin/templates/wp-handle-row-actions.php';
?>

<nav id="hierarchy-breadcrumb">
	<?php
	$view_toplevel_url = Main::get_groups_list_admin_url();
	...
	for ( $i = count( $elder_ids ) - 1; $i >= 0; --$i ) {
		...
		$view_crumb_url = Main::get_groups_list_admin_url( $crumb->ID );
